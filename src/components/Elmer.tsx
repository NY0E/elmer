...
const TOPICS = [
  {
    id: "first_qso",
    label: "Making Your First QSO",
    description: "Calling, answering, exchange flow",
    icon: "📻",
  },
  {
    id: "antenna_system",
    label: "Your Antenna System",
    description: "Feedline, connectors, SWR, what it all means",
    icon: "📡",
  },
  {
    id: "propagation",
    label: "Propagation Intuition",
    description: "Why signals go where they go, when they go there",
    icon: "🌐",
  },
  {
    id: "repeaters",
    label: "Working Repeaters",
    description: "Access tones, offsets, linking, etiquette",
    icon: "🗼",
  },
  {
    id: "troubleshooting",
    label: "Troubleshooting Your Station",
    description: "RFI, audio problems, poor reports",
    icon: "🔧",
  },
  {
    id: "vhf_practical",
    label: "VHF/UHF in Practice",
    description: "Simplex range realities, APRS, weak signal",
    icon: "📶",
  },
];
...
const TOPIC_OPENERS: Record<string, string> = {
  first_qso:
    "You're sitting at your radio. You tune to 146.520 and the band sounds clear. What do you do next?",
  antenna_system:
    "You just finished setting up your antenna and your radio is showing an SWR of 2.5:1. Walk me through what's actually happening on that feedline right now.",
  propagation:
    "It's 9pm on a Tuesday. You want to make a contact. Which band do you turn to first, and why?",
  repeaters:
    "You key up on a local repeater and get nothing back — no courtesy tone, no response. What's your first thought?",
  troubleshooting:
    "You're getting a 59 signal report but the other op says your audio sounds 'processed' and harsh. What do you start looking at?",
  vhf_practical:
    "Your HT claims 5 watts output. You're in your house trying to hit a repeater 15 miles away and you're barely getting in. Walk me through why that might be.",
};
    ...
    <div style={{ display: "flex", alignItems: "flex-end", gap: "2px", height: "20px" }}>
      {levels.map((h, i) => (
        <div
          ...
          style={{
            width: "3px",
            height: `${active ? h * 2.5 : 3}px`,
            background: active
              ? h > 6 ? "#ef4444" : h > 4 ? "#f59e0b" : "#22c55e"
              : "#374151",
  ...
  return (
    <div
      style={{
        ...
        alignItems: isElmer ? "flex-start" : "flex-end",
        marginBottom: "16px",
      }}
    >
      {isElmer && (
      ...
      <div
        style={{
          maxWidth: "82%",
          padding: "12px 16px",
          borderRadius: isElmer ? "2px 12px 12px 12px" : "12px 2px 12px 12px",
          background: isElmer ? "#1a2332" : "#1e3a2f",
          border: isElmer ? "1px solid #2d4a6b" : "1px solid #2d5a3f",
          color: isElmer ? "#c8d8e8" : "#a8d8b8",
          fontSize: "15px",
          lineHeight: "1.6",
...
export default function Elmer() {
  ...
  const startSession = (topic: Topic) => {
    ...
    const opener = TOPIC_OPENERS[topic.id];
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          ...
          openerText: TOPIC_OPENERS[selectedTopic.id],
        }),
      });
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#0b0f14", color: "#c8d8e8", fontFamily: "'Inter', sans-serif", display: "flex", flexDirection: "column" }}>
        <div style={{ borderBottom: "1px solid #1e2d3d", padding: "24px 32px", display: "flex", alignItems: "baseline", gap: "16px" }}>
          <div style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: "28px", color: "#f59e0b", letterSpacing: "0.08em" }}>
            ELMER
          </div>
          <div style={{ fontSize: "12px", color: "#4a6a8a", letterSpacing: "0.15em", textTransform: "uppercase" }}>
            Practical Ham Radio · Socratic Tutor
          </div>
        </div>

        <div style={{ padding: "48px 32px 32px", maxWidth: "680px" }}>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0b0f14", color: "#c8d8e8", fontFamily: "'Inter', sans-serif", display: "flex", flexDirection: "column" }}>
      <div style={{ borderBottom: "1px solid #1e2d3d", padding: "14px 24px", display: "flex", alignItems: "center", gap: "16px", background: "#0b0f14", position: "sticky", top: 0, zIndex: 10 }}>
        <button
          onClick={() => { setScreen("home"); setMessages([]); setInput(""); setError(""); }}
        ...
        <div style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: "13px", color: "#f59e0b", letterSpacing: "0.06em" }}>
          {selectedTopic?.icon} {selectedTopic?.label.toUpperCase()}
        </div>
